...
public class SortedComparableList {
    ...
    public void insert(Comparable c) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
        SortedComparableList listPointer = this;
        SortedComparableList listBehind = null;
        SortedComparableList insertedList = null;
        while (c.compareTo(listPointer.head) < 0) {
            listBehind = listPointer;
            listPointer = listPointer.tail;
        }
        if (listBehind == null) { // if insert before first element
            listPointer = listPointer.tail;
            insertedList = new SortedComparableList(c, listPointer);
            Comparable temp = insertedList.head;
    ...
    public void extend(SortedComparableList that) {
        // REPLACE THIS LINE WITH YOUR SOLUTION
        SortedComparableList listPointer = that;
        while (listPointer != null) {
            insert(listPointer.head);
            listPointer = listPointer.tail;
        }
    }
    ...
    public static SortedComparableList sublist(SortedComparableList L, int start, int len) {
        ...
        SortedComparableList tempTail = subTail(L, start);
        SortedComparableList listToReturn = new SortedComparableList(tempTail.head, null);
        SortedComparableList listPointer = listToReturn;
        for (int i = 0; i < len - 1; i++) {
            tempTail = tempTail.tail;
            SortedComparableList restList = new SortedComparableList(tempTail.head, null);
            listPointer.tail = restList;
            listPointer = listPointer.tail;
        }
        return listToReturn;
    }
    ...
    public void squish() {
        ...
        Comparable currentHead = this.head;
        while (temp != null) {
            if (temp.tail.head.compareTo(currentHead) == 0) {
                temp.tail = temp.tail.tail;
                break;
            }
            currentHead = temp.tail.head;
            temp = temp.tail;
        }
    }
    ...
    public void twin() {
        ...
        SortedComparableList temp = this;
        while (temp != null) {
            SortedComparableList newList = new SortedComparableList(temp.head, temp.tail);
            temp.tail = newList;
            temp = temp.tail.tail;
        }
    }
