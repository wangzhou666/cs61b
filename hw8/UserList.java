...
public class UserList {
    ...
    public static void partition(String sortFeature, CatenableQueue<User> qUnsorted, int pivot, 
        ...
        if (sortFeature.equals("id")) {            
            while (!qUnsorted.isEmpty()) {
                currentUser = qUnsorted.dequeue();
                if (currentUser.getId() < pivot) {
                    qLess.enqueue(currentUser);
                } else if (currentUser.getId() == pivot) {
                    qEqual.enqueue(currentUser);
                } else {
                    qGreater.enqueue(currentUser);
                }
            }
        } else if (sortFeature.equals("pages")) {
            while (!qUnsorted.isEmpty()) {
                currentUser = qUnsorted.dequeue();
                if (currentUser.getPagesPrinted() < pivot) {
                    qLess.enqueue(currentUser);
                } else if (currentUser.getPagesPrinted() == pivot) {
                    qEqual.enqueue(currentUser);
                } else {
                    qGreater.enqueue(currentUser);
                }
            ...
            System.out.println("Invalid feature: " + sortFeature);
        }
    }
    ...
    **/
    public static void quickSort(String sortFeature, CatenableQueue<User> q){ 
        ...
        if (sortFeature.equals("id")) {
            ...
            partition(sortFeature, q, q.nth(0).getId(), qLess, qEqual, qGreater);
            if (qLess.size() > 0) {
            ...
            CatenableQueue<User> qEqual = new CatenableQueue<User>();
            CatenableQueue<User> qGreater = new CatenableQueue<User>();
            partition(sortFeature, q, q.nth(0).getPagesPrinted(), qLess, qEqual, qGreater);
            if (qLess.size() > 0) {
                quickSort(sortFeature, qLess);
            }
            if (qGreater.size() > 0) {
    ...
    **/
    public CatenableQueue<CatenableQueue<User>> makeQueueOfQueues(){
        //Replace with solution.
        CatenableQueue<CatenableQueue<User>> theQueueOfQueues = new CatenableQueue<CatenableQueue<User>>();
        for (int i = 0; i < userQueue.size(); i++) {
            CatenableQueue<User> singleUserQueue = new CatenableQueue<User>();
            singleUserQueue.enqueue(userQueue.nth(i));
            theQueueOfQueues.enqueue(singleUserQueue);
        }
        return theQueueOfQueues;
    }
    ...
    **/
    public static CatenableQueue<User> mergeTwoQueues(String sortFeature, CatenableQueue<User> q1, CatenableQueue<User> q2){
        //Replace with solution.
        CatenableQueue<User> combinedQueue = new CatenableQueue<User>();
        if (sortFeature.equals("id")) {
            while (true) {
                if (q1.size() > 0 & q2.size() > 0) {
                    if (q1.nth(0).getId() < q2.nth(0).getId()) {
                        combinedQueue.enqueue(q1.dequeue());
                    } else {
                        combinedQueue.enqueue(q2.dequeue());
                    }
                } else if (q1.size() > 0) {
                    combinedQueue.append(q1);
                } else if (q2.size() > 0) {
                    combinedQueue.append(q2);
                } else {
                    break;
                }
            }
            ...
            while (true) {
                if (q1.size() > 0 & q2.size() > 0) {
                    if (q1.nth(0).getPagesPrinted() < q2.nth(0).getPagesPrinted()) {
                        combinedQueue.enqueue(q1.dequeue());
                    } else {
                        combinedQueue.enqueue(q2.dequeue());
                    }
    ...
    **/
    public void mergeSort(String sortFeature){
        //Replace with solution.
        CatenableQueue<CatenableQueue<User>> unsortedQ = makeQueueOfQueues();
        while (unsortedQ.size() > 1) {
            int combineCount = unsortedQ.size() / 2;
            for (int i = 0; i < combineCount; i++) {
                CatenableQueue<User> q1 = unsortedQ.dequeue();
                CatenableQueue<User> q2 = unsortedQ.dequeue();
                CatenableQueue<User> qCombined = mergeTwoQueues(sortFeature, q1, q2);
                unsortedQ.enqueue(qCombined);
            }
        }
        while (!userQueue.isEmpty()) {
            userQueue.dequeue();
        }
        while (!unsortedQ.nth(0).isEmpty()) {
            userQueue.enqueue(unsortedQ.nth(0).dequeue());
        }
    }
